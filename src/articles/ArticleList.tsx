...
const ArticleList = () => {
  ...
  useEffect(() => {
    dispatch(getNews(selectedValue));
  }, []);
  ...
  const sortArticles = (): Article[] => {
    let sorted: Article[] = [...news];
    if (sortOption === "newest") {
      sorted = sorted.sort((a, b) => new Date(b.publishedAt!).getTime() - new Date(a.publishedAt!).getTime());
    } else if (sortOption === "oldest") {
      sorted = sorted.sort((a, b) => new Date(a.publishedAt!).getTime() - new Date(b.publishedAt!).getTime());
    }
    return sorted;
  };
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto flex justify-between border-t border-gray-300/60 items-center px-5">
      ...
      <input
        ...
        />
        <select className="p-2 w-max md:w-max rounded-lg bg-gray-200/40 dark:bg-gray-600/40"
         value={selectedValue} 
